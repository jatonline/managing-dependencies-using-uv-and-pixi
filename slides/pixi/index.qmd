---
title: Can others reproduce<br>your `conda` results?<br><small>Using `pixi` in a<br>Python data analysis project</small>
...
* `python -V`
* `python my_script.py`
* Now want to do some more analysis using Pandas
...
* But others will probably forget to commit, or forget the arguments
* And can't recreate using the same versions of the dependencies
* So should really export everything, and/or at some point will likely get a full export from someone else
:::

## Someone else updates the project<br><small>(and they're using another platform)</small>
...
* <https://pixi.sh/latest/> and `curl -fsSL https://pixi.sh/install.sh | sh`
* `pixi init`
...
* `ls -A`
* `cat pixi.toml`
* `cat pixi.lock` (a YAML file with all the matching versions of the packages installed, plus hashes)
...
## Making changes to the project<br><small>(adding new dependencies and new developers)</small>

::: {.notes}
* Now want to add pandas
* `pixi add pandas`
* `cat pixi.toml`
* `pixi list`
* Now some new developers, on Mac and Windows - could edit `pixi.toml` by hand, but can use the CLI
...
* `tree` to show layout
* `cat pyproject.toml` (similar to `pixi.toml` however sections start with `tool.pixi.` - talk through, however unlikely time to go through everything)
* `pixi run lab`
...
* Global tools
    ...
    * `pixi tool install quarto` (then just `quarto`)
...
* When working with someone<br>**new to Python**
    * `pixi run dashboard`<br>oenly command they need?

* **Don't** use `conda`/`pip` by themselves
...
<small>DOI: [... ... ...](https://doi.org/10.5281/zenodo.14837979)</small>
:::
